...
- Cloud-init downloads `hc-http-gw` (pinned via `HC_HTTP_GW_VERSION` env in `holochain-first-boot`) and installs an idempotent `/usr/local/bin/hc-http-gw-configure` helper for operators to bind app ids / fn allowlists post-install. The systemd unit is `ConditionPathExists=`-gated so the rest of cloud-init still succeeds even when the binary is absent.
- `.github/workflows/release-hc-http-gw.yml` — `workflow_dispatch`-triggered mirror that builds upstream `holochain/hc-http-gw` at a chosen tag (on `ubuntu-22.04` for broad glibc compat) and uploads the binary + SHA-256 to a heart release `hc-http-gw-<upstream-tag>`. Stopgap until [`doc/upstream-hc-http-gw-release-todo.md`](doc/upstream-hc-http-gw-release-todo.md) lands; cloud-init now consumes the mirror URL so droplet boots are unblocked.
